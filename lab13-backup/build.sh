...
if [ "${SEL_N:-0}" = "0" ]; then
  ...
  if aws backup create-backup-selection --backup-plan-id "$PLAN_ID" \
       --backup-selection "$SEL_JSON" >/dev/null 2>&1; then
...
body_has()  { curl -fsS --max-time 25 "$1" 2>/dev/null | grep -q "$2"; }
...
if [ -n "${CLOUDFRONT_DOMAIN:-}" ]; then
  ...
  t "CloudFront → 정적(S3)"   body_has "https://${CLOUDFRONT_DOMAIN}/static/index.html" -i
