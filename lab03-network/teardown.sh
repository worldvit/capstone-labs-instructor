...
confirm_destroy "NAT 4개·EIP·라우팅 테이블·SG·NACL·IGW를 삭제합니다."

for k in NAT_SVC_A NAT_SVC_C NAT_MGMT_A NAT_MGMT_C; do
  id="${!k:-}"; [ -n "$id" ] || continue
...
for k in IGW_SVC IGW_MGMT NAT_SVC_A NAT_SVC_C NAT_MGMT_A NAT_MGMT_C NAT_SVC_A_EIP NAT_SVC_C_EIP NAT_MGMT_A_EIP NAT_MGMT_C_EIP \
         ...
         SG_ALB SG_APP SG_DB SG_BASTION SG_VPCE NACL_SVC_APP LAB03_DONE; do
  drop_state "$k"
done
